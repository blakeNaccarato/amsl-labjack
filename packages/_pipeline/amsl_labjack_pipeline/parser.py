...
from amsl_labjack_pipeline.models.path import get_amsl_labjack_pipeline_context
...
def invoke(
    ...
    *,
    ...
    from amsl_labjack_pipeline.cli import AmslLabJackPipeline, Stage  # noqa: PLC0415

    if isinstance(instance, ContextStore):
        instance.context = get_first_innermost_context(instance)
    elif isinstance(instance, AmslLabJackPipeline) and isinstance(
...
def get_first_innermost_context(model: ContextStore) -> AmslLabJackPipelineContexts:
    ...
    while isinstance(
        ...
        context: Context = dict(m).get(CONTEXT, get_amsl_labjack_pipeline_context())
